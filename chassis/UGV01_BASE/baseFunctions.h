...
void imuInfo(){
//  Serial.println("Sending IMU info!");
  jsonInfoSend.clear();
//  jsonFeedbackWeb = "";
...
//  Serial.print("TEMP: ");
//  Serial.println(jsonInfoSend["temp"].as<float>());
  
  if (serializeJson(jsonInfoSend, Serial) == 0) {
    Serial.println("Failed to serialize JSON");
  } else {
    Serial.println("JSON serialized successfully");
  }
...
void serialCtrl(){
  // if msg in serial buffer, read it and do stuff
  if (Serial.available()){
